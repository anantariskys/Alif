...
<body class="bg-gray-100">
    <div class="container mx-auto py-8">
        <h1 class="text-3xl font-bold mb-4 muted">Tambah Skill</h1>
        <form action="{{ route('skills.store') }}" method="POST" enctype="multipart/form-data" class="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4">
            @csrf
            <div class="mb-4">
            ...
            <div class="mb-4">
                ...
                <textarea class="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline @error('misal') border-red-500 @enderror" id="misal" name="misal" rows="3" required>{{ old('misal') }}</textarea>
                @error('misal')
                    <p class="text-red-500 text-xs italic">{{ $message }}</p>
                @enderror
            </div>
